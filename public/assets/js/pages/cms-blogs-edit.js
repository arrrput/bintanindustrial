    ...
    function removeOldImage(path, id) {
        const input = document.createElement('input');
        input.type = 'hidden'; input.name = 'deleted_images[]'; input.value = path;
    ...
    function previewImages(event) {
        ...
        // We keep existing images, just preview new ones
        Array.from(event.target.files).forEach(file => {
            ...
            reader.onload = e => {
                ...
                img.classList.add("rounded", "border", "shadow-sm", "opacity-75");
