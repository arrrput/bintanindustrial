...
@section('title', 'Add Facilities & Work Content - BIIE CMS')
  ...
  <div class="container d-lg-flex justify-content-between align-items-center">
    <nav class="breadcrumbs">
      <ol>
        ...
        <li class="current">Add Work Item</li>
      </ol>
    </nav>
  </div>
</div>
        ...
        <div class="col-lg-10">
            <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
                <div class="card-header bg-success py-3 border-0">
                    <h5 class="mb-0 text-white fw-bold"><i class="fa-solid fa-plus-circle me-2"></i> Add Facilities & Work Content</h5>
                </div>
                <div class="card-body p-4 p-md-5">
                    <form action="{{ route('cms.bies.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="page_group" value="work">

                        <div class="row g-4">
                            <!-- Title -->
                            <div class="col-md-8">
                                <label class="form-label fw-bold text-dark">Title <span class="text-danger">*</span></label>
                                <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" placeholder="e.g., Prepared LAND PARCELS" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="category" id="categorySelect" class="form-select @error('category') is-invalid @enderror" required onchange="toggleFields()">
                                    <option value="main_section" {{ old('category') == 'main_section' ? 'selected' : '' }}>Main Section (Full Width)</option>
                                    <option value="service_suite" {{ old('category') == 'service_suite' ? 'selected' : '' }}>Service Suite (Card)</option>
                                </select>
                            ...
                            <div id="subtitleField" class="col-md-12">
                                ...
                                <input type="text" name="subtitle" class="form-control @error('subtitle') is-invalid @enderror" value="{{ old('subtitle') }}" placeholder="e.g., Built for fast start-ups...">
                            ...
                            <div id="iconField" class="col-md-4 d-none">
                                ...
                                @include('cms.layouts.icon-picker-input', [
                                    ...
                                    'value' => old('icon', 'fa-solid fa-gear')
                                ])
...
@push('scripts')
<script src="{{ asset('assets/js/pages/cms-bies-create-work.js') }}"></script>
@endpush
